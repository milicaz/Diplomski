...
const ResetPasswordPage = () => {
  ...
  const handleResetPassword = async () => {
    ...
    if (!confirmPassword) {
      newError.confirmPassword = t(
        "reset-password-page.error.confirmPasswordRequired"
      );
      valid = false;
    ...
    if (valid) {
      ...
      try {
        const response = await httpPublic.put(
          `/password-reset/reset?token=${encodeURIComponent(
            token
