...
public class JwtUtils {
	...
	public String generateJwtToken(ZaposleniDetailsImpl zaposleniPrincipal) {
		List<String> authorities = zaposleniPrincipal.getAuthorities().stream().map(GrantedAuthority::getAuthority)
				.collect(Collectors.toList());
		return Jwts.builder().setSubject(zaposleniPrincipal.getEmail()).claim("userType", "Zaposleni").claim("roles", authorities).setIssuedAt(new Date())
