...
const Pagination = ({
  pages,
  currentPage, // Receive currentPage from parent
  setCurrentPage,
  array,
  limit,
  maxVisibleButtons,
  ...
  // Sync currentButton with currentPage when currentPage changes
  useEffect(() => {
    setCurrentButton(currentPage);
  }, [currentPage]);

  // Update currentPage whenever currentButton changes
  useEffect(() => {
    setCurrentPage(currentButton);
  }, [currentButton, setCurrentPage]);

  useEffect(() => {
    // Set currentButton to 1 if pages is less than the currentButton
    if (pages < currentButton) {
      setCurrentButton(pages);
    }
  }, [pages, currentButton]);
  ...
  useEffect(() => {
    // Reset currentButton to 1 on initial load or reload
    setCurrentButton(1);
